...
- **Python 3.12**（3.13以降はPyTorchが未対応）
...
# PyTorch CPU版をインストール（GPU不要なら軽量版を推奨）
...
```bash
python run.py --history          # 実行一覧を表示
python run.py --history 3        # Run ID 3 の詳細を表示
```

パイプラインの実行結果はSQLite（`data/runs.db`）に自動保存されます。

### 結果の評価とインサイト

```bash
python run.py --rate 1 5 "臓器移植のアナロジーが実務に刺さった"
python run.py --rate 2 2 "表面的な類似に引っ張られていた"
python run.py --insights         # ドメイン別のフィードバック統計を表示
```

フィードバックが蓄積されると、どの出自ドメインからのアナロジーが実際に役立つかの傾向が見えてきます。
...
[Step 1] 構造抽出 ――― 課題の因果・依存関係を構造化
    |
    v
[Step 2] 類似構造探索 ――― 近縁1 + 異分野4を発見（件数は環境変数で調整可能）
    |
    v
[Step 3] Semantic SME ――― テキスト類似度 + グラフ構造類似度(GNN)の複合スコアリング
    |
    v
...
============================================================
[Step 3] Semantic SME（5候補をスコアリング）
============================================================
...
```
.
├── run.py                          # エントリポイント（--history, --rate, --insights）
├── src/
│   ├── config.py                   # 設定（環境変数、閾値）
│   ├── models.py                   # データ構造（TypedDict）
│   ├── pipeline.py                 # パイプライン全体の実行制御
│   ├── db.py                       # SQLite永続化（薄いDAO層 + フィードバック）
│   ├── idea_bank.py                # アイデアバンク（出自剥離）
│   ├── graph_embedding.py          # GNNグラフ構造埋め込み
│   ├── clients/
...
│       ├── step3_sme.py            # Semantic SME（テキスト + グラフ複合スコアリング）
...
| 埋め込み | sentence-transformers (paraphrase-multilingual-MiniLM-L12-v2) | 関係レベルのテキストマッチング |
| グラフ | PyTorch Geometric (GCN) | 固定重みGNNによるグラフ構造類似度 |
| 永続化 | SQLite（WALモード） | 実行結果の自動保存 + フィードバック学習基盤 |
...
- **テキスト埋め込み + GNNグラフ埋め込みの複合で構造的類似をランキングする**（Step 3のSemantic SME）
...
- **Phase 1**（完了）: End-to-Endパイプラインの構築と検証
- **Phase 2**（完了）: ハンガリアン法による排他的マッチング、SQLite永続化、GNNグラフ埋め込みによる構造的類似度、フィードバックループ（評価記録・統計表示）
- **Phase 3**: 感情メモリシステムとの統合、フィードバック駆動の探索最適化、外部発信
